...
from utils.report_reserva import generar_pdf
from utils.gmail import enviar_correo

def smoke_test_reserva():
    ...
    resultados = []
    capturas = []
    url = "http://localhost:3000/php/reserva.php?habitacion_id=24"
    ...
    try:
        ...
        campos = {
            "Fecha inicio (check in)": "fecha_inicio",
            "Fecha fin (check on)": "fecha_fin",
        }
    ...
    asunto = "Reporte de Smoke Test - Hacer check in y check on"
    cuerpo = f"""
    Este es el testeo de Creación de check in y check on.
