...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Pruebas automatizadas para Registro de Huésped</p>
        </div>
...
def generar_pdf(texto_resultados, capturas, archivo_salida, duracion, fecha_hora):
    ...
    pdf.cell(0, 10, "Reporte de Smoke Test - Registro Huésped", ln=True, align="C")
    pdf.ln(10)
...
def smoke_test():
    ...
    resultados = []
    capturas = []
    url = "http://localhost:3000/php/registro_huesped.php?rol=2"
        ...
        for label, name in campos.items():
            try:
                ...
                if name == "foto":
                    ...
                    if input_type == "file":
                        resultados.append(f"V Campo '{label}' (tipo archivo) encontrado.")
                    else:
                        resultados.append(f"F Campo '{label}' encontrado, pero no es tipo archivo.")
                else:
    ...
    # Enviar correo
    asunto = "Reporte Smoke Test - Registro de usuario (huésped)"
    cuerpo = "Este es el testeo de registro de usuario (huésped). Adjuntamos el reporte con capturas."
